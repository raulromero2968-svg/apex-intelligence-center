      ...
      <div className="h-64">
        <ResponsiveContainer>
          <ScatterChart margin={{ top: 10, right: 10, bottom: 10, left: 10 }}>
            ...
            <XAxis type="number" dataKey="x" name="Score" unit="" tick={{ fill: "rgba(255,255,255,0.7)" }} />
            <YAxis type="number" dataKey="y" name="Return" unit="%" tick={{ fill: "rgba(255,255,255,0.7)" }} />
            <ZAxis type="number" dataKey="z" range={[30, 160]} />
            <Tooltip cursor={{ strokeDasharray: "3 3" }} contentStyle={{ background: "rgba(17,24,39,0.9)", border: "1px solid rgba(255,255,255,0.12)", color: "#fff" }} />
            <Scatter name="Cards" data={data} fill="#22d3ee" />
          </ScatterChart>
        </ResponsiveContainer>
      </div>
      <p className="mt-3 text-xs text-white/60">
        Tip for teens: dots higher and to the right often mean stronger potential—but bigger swings. Start small, track results, and let data be your coach.
      </p>
    </div>
  );
}
