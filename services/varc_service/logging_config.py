"""Structured logging configuration for VARC service."""
...
class StructuredFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        log_data = {
            ...
            "service": getattr(record, "service", "varc_service"),
...
class TraceContextFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if not hasattr(record, "traceId"):
            # Try to get from contextvars or thread-local storage
            # For simplicity, we'll rely on explicit setting via adapter
            pass
        return True


def setup_logging(
    service_name: str = "varc_service",
    log_level: str = "INFO",
...
def get_logger(name: Optional[str] = None) -> logging.Logger:
    ...
    Args:
        name: Logger name (defaults to 'varc_service')
...
class TraceLoggerAdapter(logging.LoggerAdapter):
    ...
    def process(self, msg: str, kwargs: dict) -> tuple[str, dict]:
        ...
        if trace_id:
            ...
            extra["service"] = self.extra.get("service", "varc_service")
            kwargs["extra"] = extra
        return msg, kwargs


def get_trace_logger(trace_id: Optional[str], service_name: str = "varc_service") -> TraceLoggerAdapter:
    """
