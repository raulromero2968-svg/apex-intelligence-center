...
import { contrarianCompletedChannel, ContrarianResultPayload } from '@apex/shared/src/contracts/queues';
    ...
    async start(controller) {
      ...
      const messageHandler = (ch: string, message: string) => {
        if (ch === channel && !closed) {
          try {
            const result: ContrarianResultPayload = JSON.parse(message);
            ...
            console.error('[contrarian-sse] Failed to parse message:', error);
          }
        }
      };
